module.exports = {
  ...
  definitions: {
    ...
    'close-tab': {
      type: 'object',
      description:
        'Close the current tab. Badgecount is the number of not-accepted testcases, but you can overwrite this.',
      required: ['command'],
      properties: {
    ...
    'close-judgement': {
      type: 'object',
      description:
        'Close the current judgement. Accepted iff all contexts are accepted, status is the worst (highest in description) of all tests, summary is the last of all tests, but you can overwrite this.',
      required: ['command'],
      properties: {
    ...
    format: {
      type: 'string',
      description:
        "Either 'plain' (or 'text'), 'html' (formatted), 'markdown' (formatted), 'callout' (formatted), 'code' or any programming language (highlighted). Note that the HTML output is sanitised to prevent XSS issues. JavaScript, for example, is not allowed, but most other tags should work.",
    },
    permission: {
      type: 'string',
      description: 'To which level of user this message is visible.',
      enum: ['student', 'staff', 'zeus'],
    },
    badgeCount: {
      type: 'number',
