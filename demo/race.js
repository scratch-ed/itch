...
function checkStartCarConditions(sprite, template, submission, l) {
  ...
  // Check hat
  const templateStartIndex = fromTemplate.blocks.findIndex(
    (b) => b.opcode === 'event_whenflagclicked',
  );
  const submissionStartIndex = fromSubmission.blocks.findIndex(
    (b) => b.opcode === 'event_whenflagclicked',
  );
  const templateBlocks = fromTemplate.blocks.slice(
    templateStartIndex,
    templateStartIndex + 6,
  );
  const submissionBlocks = fromTemplate.blocks.slice(
    submissionStartIndex,
    submissionStartIndex + 6,
  );
...
function beforeExecution(template, submission, e) {
  e.describe('Controle op bestaande code', (l) => {
    ...
    l.test('Bliksem', (l) => {
      ...
      const templateBlocks = fromTemplate.blocks.slice(
        templateStartIndex,
        templateStartIndex + 4,
      );
      const submissionBlocks = fromTemplate.blocks.slice(
        submissionStartIndex,
        submissionStartIndex + 4,
      );
      l.expect(submissionBlocks)
        .with({
    ...
    others.forEach((sprite) => {
      l.test(sprite, (l) => {
        checkExistence(l, submission, sprite);
        l.expect(submission.sprite(sprite).blocks).toBe(
          template.sprite(sprite).blocks,
        );
      });
    });
  });
    ...
    event
      ...
      .log(() => {
        ...
        e.test(`Herhalende lus gebruikt bij ${name}`, (l) => {
          const blocks = car
            .blockList()
            .filter((bl) => bl.opcode === 'control_repeat_until');
          const found = blocks.some((block) => {
            ...
            const child = car
              ...
              .find(
                (bl) =>
                  bl.opcode === 'sensing_touchingobject' &&
                  bl.parent === block.id,
              );
            return (
              car
                ...
                .find(
                  (bl) =>
                    bl.opcode === 'sensing_touchingobjectmenu' &&
                    bl.fields.TOUCHINGOBJECTMENU?.value === 'Eindmeet' &&
                    bl.parent === child.id,
                ) !== undefined
            );
          });
      ...
      .log(() => {
        e.test(`${name} kan omhoog bewegen`, (l) => {
          const event = e.log.events.list.find((ev) => {
            return (
              ev.time > start && ev.type === 'useKey' && ev.data.key === keys.up
            );
          });
          const before = event.previousFrame.getSprite(name);
          const after = event.nextFrame.getSprite(name);
      ...
      .log(() => {
        ...
        const normalStartFrame = e.log.events.list.find(
          (ev) =>
            ev.time >= start &&
            ev.type === 'useKey' &&
            ev.data.key === keys.down,
        ).previousFrame;
        const touchGrassFrame = e.log.events.list.find(
          (ev) =>
            ev.time > start &&
            ev.type === 'waitForSpriteTouch' &&
            ev.data.sprite === name,
        ).nextFrame;
      ...
      .log(() => {
        ...
        e.test(`${name} moet 3s wachten bij het botsen met de boom`, (l) => {
          const touchFrame = e.log.events.list.find(
            (ev) =>
              ev.type === 'waitForSpriteTouch' &&
              ev.data.targets[0] === 'Boom' &&
              ev.data.sprite === name,
          )?.nextFrame;
          const touchSprite = touchFrame.getSprite(name);
          const moveFrames = e.log.frames.filter(
            (fr) => fr.time > touchFrame.time && fr.block === `update_${name}`,
          );
          ...
          if (!touchFrame || !firstMoved) {
            ...
            l.expect(
              expected - expected / 2 <= time &&
                time <= expected + expected / 2,
            )
              .with({
                correct: `Goed zo, ${name} wacht 3 seconden wanneer hij tegen de boom botst.`,
                wrong: `Oeps, als ${name} de boom raakt dan moet hij 3 seconden blijven staan.`,
        ...
        e.test(
          `${name} moet achteruit bewegen na het botsen met de boom`,
          (l) => {
      ...
      .log(() => {
        e.test(`${name} zegt iets als de eindmeet bereikt wordt`, (l) => {
          const event = e.log.events.list.find(
            (ev) =>
              ev.type === 'say' && ev.time > start && ev.data.sprite === name,
          );
          l.expect(event)
            .with({
      ...
      .log(() => {
        e.test(
          `${name} blijft een seconde staan na botsing met de rots`,
          (l) => {
            // Check that the car waits 1000ms before moving.
            const touchFrame = e.log.events.list.find(
              (ev) =>
                ev.type === 'waitForSpriteTouch' &&
                ev.data.targets[0] === 'Rots' &&
                ev.data.sprite === name,
            ).nextFrame;
            const touchSprite = touchFrame.getSprite(name);
            const moveFrames = e.log.frames.filter(
              (fr) =>
                fr.time > touchFrame.time && fr.block === `update_${name}`,
            );
            const firstMoved = moveFrames.find((fr) => {
              const sp = fr.getSprite(name);
              return sp.x !== touchSprite.x || sp.y !== touchSprite.y;
            ...
            if (!touchFrame || !firstMoved) {
              l.expect(touchFrame && firstMoved)
                .with({
                  correct:
                    'Joepie! Je auto moet 1 seconde wachten voordat hij terug kan rijden.',
                  wrong:
                    'Ai, probeer eerst een seconde te wachten voordat de auto terug mag rijden na een botsing met de rots.',
                })
                .toBe(true);
            } else {
              const time = firstMoved.time - touchFrame.time;
              ...
              l.expect(
                ...
                .with({
                  correct:
                    'Joepie! Je auto moet 1 seconde wachten voordat hij terug kan rijden.',
                  wrong:
                    'Ai, probeer eerst een seconde te wachten voordat de auto terug mag rijden.',
                })
                .toBe(true);
            }
          },
        ...
        e.test(`${name} gaat terug naar start bij botsing met rots`, (l) => {
          ...
          l.expect(sprite.x > -200 && sprite.x < -150 && sprite.y > 150)
            .with({
              correct:
                'Super! De auto gaat terug naar de startplaats wanneer hij de rots raakt.',
              wrong:
                'Laat de auto naar de startplaats gaan nadat hij 1 seconde heeft gewacht bij de rots.',
            })
            .toBe(true);
        });
      })
      ...
      .log(() => {
        ...
        e.test(`Bliksem verplaatst ${name} naar ergens anders`, (l) => {
          ...
          l.expect(distance > 50)
            .with({
              correct:
                'Joepie! Wanneer de auto geraakt is door de bliksem, dan wacht hij eventjes voordat hij verdwijnt en verschijnt op een nieuwe plaats.',
              wrong:
                'Als de auto geraakt wordt door de bliksem, dan moet hij 0.5 seconden wachten, verdwijnen en verschijnen op een andere plaats.',
            })
            .toBe(true);
        });
        e.test(`Bliksem gaat weg nadat ${name} er tegen rijdt`, (l) => {
          ...
          l.expect(sprite.visible)
            .with({
              ...
              wrong:
                'De bliksemschicht moet wachten tot hij de blauwe of de gele auto raakt, dan moet hij 0.1 seconden wachten en verdwijnen.',
            })
            .toBe(false);
        });
      })
      ...
      .log(() => {
        e.test(
          `${name} gaat naar willekeurige posities na het aanraken van de vat`,
          (l) => {
            const frames = e.log.frames.filter(
              (ev) => ev.time >= start && ev.block === `update_${name}`,
            );
            const positions = e.log.getSpritePositions(name, frames);
            l.expect(positions.length >= 2)
              .with({
                correct:
                  'Goed zo! De auto richt zich naar een willekeurige richting wanneer hij het lekkend vat raakt.',
                wrong:
                  'Probeer ervoor te zorgen dat de auto zich naar een willekeurige richting richt en vervolgens stappen neemt als hij het vat raakt.',
              })
              .toBe(true);
          },
        );
