...
import { getExerciseLinkData } from './link-template-and-solution.js';
...
program
  ...
  .action(async (local) => {
    ...
    const exercises = fs
      .readdirSync(local, { withFileTypes: true })
      .filter((p) => p.isDirectory());
    const data = [['oefening id', 'naam', 'starter id', 'oplossing id']];
    for (const exercise of exercises) {
      const absolutePath = `${local}/${exercise.name}`;
      const relativePath = exercise.name;
      const result = await getExerciseLinkData(absolutePath, relativePath, token);
      data.push(...result);
    }
    const csv = data.map((row) => row.join(',')).join('\r\n');
    fs.writeFileSync('link.csv', csv);
  });
